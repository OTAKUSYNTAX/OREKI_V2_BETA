...
async function handleDeletedMessage(message) {
    try {
        ...
        if (msg.imageMessage) {
            mediaUrl = await ThugBotInc.downloadAndSaveMediaMessage(msg.imageMessage);
            caption = msg.imageMessage.caption || caption; // Use image caption if available
        }

        // Check if it's a video
        if (msg.videoMessage) {
            mediaUrl = await ThugBotInc.downloadAndSaveMediaMessage(msg.videoMessage);
        ...
        await ThugBotInc.sendMessage(ThugBotInc.user.id, {
...
async function deleteUpdate(message) {
    try {
        ...
        let chats = global.db.data.chats[msg.chats] || {}
            ...
            await this.reply(ThugBotInc.user.id, `
            ≡ deleted a message 
            ┌─⊷  𝘼𝙉𝙏𝙄 𝘿𝙀𝙇𝙀𝙏𝙀 
            ▢ *Number :* @${participant.split`@`[0]} 
            └─────────────
        ...
        this.copyNForward(ThugBotInc.user.id, msg, false).catch(e => console.log(e, msg))
    } catch (e) {
        console.error(e)
    }
}
